...
nbac17to20 <-
  unlist(
    readOGR(
      'nbac2017_to_2020_20210810.shp'
    )
  )
mtbs17to20 <-
  unlist(
    readOGR(
      'mtbs_2017_to_2020.shp'
    )
  )
...
f <- function(i) {
  ...
  ext.val.bui = full_join(ext.val.bui, IDlookup) %>% dplyr::select(-ID) %>% rename(ID =
                                                                                     IDseq2) %>% dplyr::select(x, y, ID, everything())
  ...
  ext.val.dc = full_join(ext.val.dc, IDlookup) %>% dplyr::select(-ID) %>% rename(ID =
                                                                                   IDseq2) %>% dplyr::select(x, y, ID, everything())
  ...
  ext.val.dmc = full_join(ext.val.dmc, IDlookup) %>% dplyr::select(-ID) %>% rename(ID =
                                                                                     IDseq2) %>% dplyr::select(x, y, ID, everything())
  ...
  ext.val.fwi = full_join(ext.val.fwi, IDlookup) %>% dplyr::select(-ID) %>% rename(ID =
                                                                                     IDseq2) %>% dplyr::select(x, y, ID, everything())
  ...
  write.table(ext.val.bui,
              file = out.bui.name,
              sep = ',',
              row.names = FALSE) # keeps the rownames
...
start_time <- Sys.time()
NAfire_csvwrite <-
  foreach(i = 1:42, .options.snow = opts) %dopar%  f(i)#, .combine = 'rbind'
stopCluster(cl)
end_time <- Sys.time()
end_time - start_time
...
g <- function(j) {
  ...
  f_summer <- function(i) {
    ...
    if (year == 1979) {
      ...
      colnames(Fire) <- varname
      FireSummer <-
        Fire[, c(1:4, summerdays:summerdaye)] %>% mutate(var = j) %>% dplyr::select(var, everything())
    } else {
      Fire <- Fire %>% dplyr::select(-c(x, y, ID))
      if (year %% 4 == 0) {
        summerdays <- 153
  ...
  f_fall <- function(i) {
    ...
    if (year == 1979) {
      ...
      colnames(Fire) <- varname
      FireFall <-
        Fire[, c(1:4, falldays:falldaye)] %>% mutate(var = j) %>% dplyr::select(var, everything())
    } else {
      Fire <- Fire %>% dplyr::select(-c(x, y, ID))
      if (year %% 4 == 0) {
        falldays <- 245
  ...
  f_spring <- function(i) {
    ...
    if (year == 1979) {
      ...
      colnames(Fire) <- varname
      Firespring <-
        Fire[, c(1:4, springdays:springdaye)] %>% mutate(var = j) %>% dplyr::select(var, everything())
    } else {
      Fire <- Fire %>% dplyr::select(-c(x, y, ID))
      if (year %% 4 == 0) {
        springdays <- 61
  ...
  VarFire <-
    foreach(i = pathind,
            .options.snow = opts,
            .combine = 'cbind') %dopar%  f(i)#, .combine = 'rbind'
  VarFire_summer <-
    foreach(i = pathind,
            .options.snow = opts,
            .combine = 'cbind') %dopar%  f_summer(i)#, .combine = 'rbind'
  VarFire_fall <-
    foreach(i = pathind,
            .options.snow = opts,
            .combine = 'cbind') %dopar%  f_fall(i)#, .combine = 'rbind'
  VarFire_spring <-
    foreach(i = pathind,
            .options.snow = opts,
            .combine = 'cbind') %dopar%  f_spring(i)#, .combine = 'rbind'
...
f.f <- function(i) {
  ...
  vardf_onb = left_join(onb, vardf) %>% mutate(varmatch = paste(varname, year, day, sep =
                                                                  '_')) %>% dplyr::select(varmatch, everything())
  ...
  vardf_onb <-
    vardf_onb %>% group_by(year, seq, day) %>% summarise(across(everything(),  ~ mean(., na.rm = TRUE))) %>% dplyr::select(onbvalue, everything())
  ...
  quanfirst21 = apply(vardf_onb_first21_mat, 1, function(x)
    ecdf(x[2:ncol(vardf_onb_first21_mat)])(x[1]))
  quanlater21 = apply(vardf_onb_later21_mat, 1, function(x)
    ecdf(x[2:ncol(vardf_onb_later21_mat)])(x[1]))
...
quan_daily <-
  foreach(i = 1:4,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f.f(i)#, .combine = 'rbind'
stopCluster(cl)
end_time <- Sys.time()
end_time - start_time

quan_daily_long = quan_daily %>% gather(key = 'variable',
                                        value = 'value',
                                        quanfirst21_BUI:quanlater21_FWI)

ggplot(data = quan_daily_long) + geom_density(aes(x = value, fill = variable, ..scaled..), alpha =
                                                0.6) + scale_fill_npg() +
...
quanfirst21 = quan_daily %>% dplyr::select(quanfirst21_FWI,
                                           quanfirst21_BUI,
                                           quanfirst21_DMC,
                                           quanfirst21_DC,
                                           id)
quanlater21 = quan_daily %>% dplyr::select(quanlater21_FWI,
                                           quanlater21_BUI,
                                           quanlater21_DMC,
                                           quanlater21_DC,
                                           id)
...
ggplot(data = xx, aes(x = ts, y = value, fill = variable)) + geom_boxplot(size =
                                                                            0.5) +
  geom_line(aes(group = id), position = position_dodge(0.2), alpha = 0.05) +
  geom_point(
    aes(color = variable, group = id),
    size = 1,
    shape = 21,
  ...
  scale_fill_lancet() + scale_color_lancet() + envalysis::theme_publish() +
  ylab('Quantiles') + theme(axis.title.x = element_blank(), legend.position = 'none')


res <-
  wilcox.test((quanfirst21 %>% dplyr::select(BUI))$BUI,
              (quanlater21 %>% dplyr::select(BUI))$BUI,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((quanfirst21 %>% dplyr::select(FWI))$FWI,
              (quanlater21 %>% dplyr::select(FWI))$FWI,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((quanfirst21 %>% dplyr::select(DMC))$DMC,
              (quanlater21 %>% dplyr::select(DMC))$DMC,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((quanfirst21 %>% dplyr::select(DC))$DC,
              (quanlater21 %>% dplyr::select(DC))$DC,
              paired = TRUE,
              alternative = 'greater'
  )
...
f.f <- function(i) {
  ...
  onb.point.quantile42Y = apply(vardf_onb_mat, 1, function(x)
    ecdf(x[2:ncol(vardf_onb_mat)])(x[1]))
...
df.onb.p.q.f = onb.point.quantile42Y.f %>% gather(key = 'variable',
                                                  value = 'value',
                                                  Allseason_BUI:Allseason_FWI)

ggplot(data = df.onb.p.q.f) + geom_density(aes(x = value, fill = variable, ..scaled..), alpha =
                                             0.6) + scale_fill_npg() +
  envalysis::theme_publish(
    base_size = 8,
    base_family = "",
...
start_time <- Sys.time()
onb.point.q42Y.summer.f <-
  foreach(i = 1:6,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_summer.f(i)#, .combine = 'rbind'
onb.point.q42Y.fall.f <-
  foreach(i = 1:6,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_fall.f(i)#, .combine = 'rbind'
onb.point.q42Y.spring.f <-
  foreach(i = 1:6,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_spring.f(i)#, .combine = 'rbind'
stopCluster(cl)
end_time <- Sys.time()
end_time - start_time
...
ggplot(data = df.onb.p.q.summer.f) + geom_density(aes(x = value, fill =
                                                        variable, ..scaled..), alpha = 0.6) + scale_fill_npg() +
  envalysis::theme_publish(
    base_size = 8,
    base_family = "",
    line_size = 0.25,
...
ggplot(data = df.onb.p.q.fall.f) + geom_density(aes(x = value, fill = variable), alpha =
                                                  0.6) + scale_fill_npg() +
  envalysis::theme_publish(
    base_size = 8,
    base_family = "",
...
ggplot(data = df.onb.p.q.spring.f) + geom_density(aes(x = value, fill =
                                                        variable), alpha = 0.6) + scale_fill_npg() +
  envalysis::theme_publish(
    base_size = 8,
    base_family = "",
    line_size = 0.25,
...
df.onb.p.q.f <-
  df.onb.p.q.f %>% rowwise() %>%  mutate(x = strsplit(variable, '_'),
                                         season = x[1],
                                         var = x[2])
df.onb.p.q.spring.f <-
  df.onb.p.q.spring.f %>% rowwise() %>%  mutate(x = strsplit(variable, '_'),
                                                season = x[1],
                                                var = x[2])
df.onb.p.q.summer.f <-
  df.onb.p.q.summer.f %>% rowwise() %>%  mutate(x = strsplit(variable, '_'),
                                                season = x[1],
                                                var = x[2])
df.onb.p.q.fall.f <-
  df.onb.p.q.fall.f %>% rowwise() %>% mutate(x = strsplit(variable, '_'),
                                             season = x[1],
                                             var = x[2])
...
ggplot(data = df.onb.season.var.f) + geom_boxplot(aes(x = var, y = value, fill =
                                                        season), alpha = 0.6) + scale_fill_aaas() +
  envalysis::theme_publish(
    base_size = 8,
    base_family = "",
    line_size = 0.25,
...
colnames(onb.point.quantile42Y.f) <-
  c('BUI', 'DC', 'DMC', 'FWI', 'id', 'ts')
colnames(onb.point.quantile.a) <-
  c('BUI', 'DC', 'DMC', 'FWI', 'id', 'ts')
...
res <-
  t.test((onb.point.quantile42Y.f %>% dplyr::select(BUI))$BUI,
         (onb.point.quantile.a %>% dplyr::select(BUI))$BUI,
         paired = TRUE
  )
res
res <-
  t.test((onb.point.quantile42Y.f %>% dplyr::select(FWI))$FWI,
         (onb.point.quantile.a %>% dplyr::select(FWI))$FWI,
         paired = TRUE
  )
res
res <-
  t.test((onb.point.quantile42Y.f %>% dplyr::select(DMC))$DMC,
         (onb.point.quantile.a %>% dplyr::select(DMC))$DMC,
         paired = TRUE
  )
res
res <-
  t.test((onb.point.quantile42Y.f %>% dplyr::select(DC))$DC,
         (onb.point.quantile.a %>% dplyr::select(DC))$DC,
         paired = TRUE
  )
...
write.table(x,
            file = '015_OverNightBurning\\015.1_DataProcessing\\Longterm Analysis\\onb_era5point_LatLong.csv',
            sep = ',',
            row.names = FALSE) # keeps the rownames

onb_era5point_LatLong = read.csv(
  '015_OverNightBurning\\015.1_DataProcessing\\Longterm Analysis\\onb_era5point_LatLong.csv'
)
vpd <-
  mixedsort(
    dir(
      path = 'D:\\000_collections\\010_Nighttime Burning\\011_Data\\016_LongTermHourly\\hvpd',
      full.names = T,
      pattern = '(nc)$'
    )
  )
isi <-
  mixedsort(
    dir(
      path = 'D:\\000_collections\\010_Nighttime Burning\\011_Data\\016_LongTermHourly\\hisi',
      full.names = T,
      pattern = '(nc)$'
    )
  )
ffmc <-
  mixedsort(
    dir(
      path = 'D:\\000_collections\\010_Nighttime Burning\\011_Data\\016_LongTermHourly\\hffmc',
      full.names = T,
      pattern = '(nc)$'
    )
  )
rh <-
  mixedsort(
    dir(
      path = 'D:\\000_collections\\010_Nighttime Burning\\011_Data\\016_LongTermHourly\\hrh',
      full.names = T,
      pattern = '(nc)$'
    )
  )
ht <-
  mixedsort(
    dir(
      path = 'D:\\000_collections\\010_Nighttime Burning\\011_Data\\016_LongTermHourly\\ht',
      full.names = T,
      pattern = '(nc)$'
    )
  )
...
##aline with
proj4string(onb_era5point_LatLong) <-
  proj4string(vpd.brick)#NAfire_year
year = rep(1979:2020, each = 12)

xx <-
  raster::extract(
    vpd.brick,
    onb_era5point_LatLong,
    weight = T,
    df = T,
    cellnumbers = T
  )
...
f_vpd <- function(i) {
  ...
  extp2var <-
    extp2var %>% group_by(ID) %>% summarise_all(mean, na.rm = TRUE)
  
  if (i == 1) {
    extp2var = extp2var
  } else{
    extp2var = extp2var %>% dplyr::select(-c(x, y, ID))
  }
}
f_isi <- function(i) {
  library(raster)
...
hvar42Y_vpd <-
  foreach(i = 1:504,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_vpd(i)#, .combine = 'rbind'
hvar42Y_isi <-
  foreach(i = 1:504,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_isi(i)#, .combine = 'rbind'
hvar42Y_ffmc <-
  foreach(i = 1:504,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_ffmc(i)#, .combine = 'rbind'
hvar42Y_rh <-
  foreach(i = 1:504,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_rh(i)#, .combine = 'rbind'
hvar42Y_ht <-
  foreach(i = 1:504,
          .options.snow = opts,
          .combine = 'cbind') %dopar%  f_ht(i)#, .combine = 'rbind'
...
readr::write_rds(
  hvar42Y_vpd,
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_vpd.Rds"
)
readr::write_rds(
  hvar42Y_isi,
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_isi.Rds"
)
readr::write_rds(
  hvar42Y_ffmc,
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_ffmc.Rds"
)
readr::write_rds(
  hvar42Y_rh,
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_rh.Rds"
)
readr::write_rds(
  hvar42Y_ht,
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_ht.Rds"
)


hvar42Y_vpd = readr::read_rds(
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_vpd.Rds"
)
hvar42Y_isi = readr::read_rds(
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_isi.Rds"
)
hvar42Y_ffmc = readr::read_rds(
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_ffmc.Rds"
)
hvar42Y_rh = readr::read_rds(
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_rh.Rds"
)
hvar42Y_ht = readr::read_rds(
  "D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\hvar42Y_ht.Rds"
)
...
FireWeatherLongTerm <- function(i) {
  ...
  x <-
    as.data.frame(seq(
      as.POSIXct("1979-01-01 00:00:00"),
  ...
  x <-
    x %>% mutate(
      ...
      day = as.integer(data.table::yday(date)),
      hr = as.integer(format(date, "%H"))
    )
  x$long <- hvar42Y_isi$x[i]
  x$lat <- hvar42Y_isi$y[i]
  ID = hvar42Y_vpd$ID[i]
  tz <-
    tz_offset(as.Date(x$date, format = "%Y-%m-%d"),
              tz_lookup_coords(as.numeric(x$lat[i]), as.numeric(x$long[i])))#as.numeric(x$lat[1]), as.numeric(x$long[1])
  ...
  x <-
    x %>% mutate(date = as.character(date + lubridate::hours(tzoffset)))
  tz <-
    tz_offset(as.Date(x$date, format = "%Y-%m-%d"),
              tz_lookup_coords(as.numeric(x$lat[i]), as.numeric(x$long[i])))#
  
  ## sunset and sunrise
  sundata <-
    data.frame(
      date = as.Date(tz$date_time),
      lat = as.numeric(x$lat[i]),
      lon = as.numeric(x$long[i])
    )
  sunrise <-
    getSunlightTimes(data = sundata,
                     keep = "sunrise",
                     tz = tz$tz_name[1])$sunrise
  sunset <-
    getSunlightTimes(data = sundata,
                     keep = "sunset",
                     tz = tz$tz_name[1])$sunset
  ...
  tz1 <-
    tz_offset(
      as.Date(as.Date(x$date) - lubridate::hours(24), format = "%Y-%m-%d"),
      tz_lookup_coords(as.numeric(x$lat[i]), as.numeric(x$long[i]))
    )
  sundata1 <-
    data.frame(
      date = as.Date(tz1$date_time),
      lat = as.numeric(x$lat[i]),
      lon = as.numeric(x$long[i])
    )
  x$sunset_lastday <-
    as.character(getSunlightTimes(
      data = sundata1,
      keep = "sunset",
      tz = tz1$tz_name[1]
    )$sunset)
  x <-
    x %>% mutate(dorn = ifelse(((date > sunrise &
                                   date < sunset) | (date < sunset_lastday)
    ), 0, 1),
    year = ifelse(day == 1 &
                    naturalDay > 360, year - 1, year))
  ...
  FW_results = foreach(j = 1:nrow(xx123), .combine = 'rbind') %do% {
    library(dplyr)
    sunrise2sunrise <-
      x %>% filter(year == xx123$year[j], naturalDay == xx123$naturalDay[j])
    if (nrow(sunrise2sunrise) >= 20) {
      FD_info <-
        sunrise2sunrise %>% summarise(
          year = year[1],
          day = day[1],
          naturalDay = naturalDay[1],
          ID = ID,
          long = long[1],
          lat = lat[1]
        )
      FD_daymax <-
        sunrise2sunrise %>% filter(dorn == 0) %>% summarise(
          vpd_daymax = max(vpd),
          isi_daymax = max(isi),
          rh_daymax = max(rh),
          temp_daymax = max(ht),
          ffmc_daymax = max(ffmc)
        )
      ...
      FD_nightmax <-
        sunrise2sunrise %>% filter(dorn == 1) %>% summarise(
          vpd_nightmax = max(vpd),
          isi_nightmax = max(isi),
          rh_nightmax = max(rh),
          temp_nightmax = max(ht),
          ffmc_nightmax = max(ffmc)
        )
      ...
      FD_nightmin <-
        sunrise2sunrise %>% filter(dorn == 1) %>% summarise(
          vpd_nightmin = min(vpd),
          isi_nightmin = min(isi),
          rh_nightmin = min(rh),
          temp_nightmin = min(ht),
          ffmc_nightmin = min(ffmc)
        )
      ...
      FD_nightmean <-
        sunrise2sunrise %>% filter(dorn == 1) %>% summarise(
          vpd_nightmean = mean(vpd),
          isi_nightmean = mean(isi),
          rh_nightmean = mean(rh),
          temp_nightmean = mean(ht),
          ffmc_nightmean = mean(ffmc)
        )
  ...
  OBE_quanfirst21 <-
    foreach(k = 1:nrow(OBE_this), .combine = 'rbind') %do% {
      ...
      quanfirst21 = apply(OBE_quantile, 1, function(x)
        ecdf(x[2:ncol(OBE_quantile)])(x[1]))
      ...
      OBE_quantile = OBE_quantile[, 7:ncol(OBE_quantile)]
      OBE_quantile = as.matrix(t(OBE_quantile))
      quanlater21 = apply(OBE_quantile, 1, function(x)
        ecdf(x[2:ncol(OBE_quantile)])(x[1]))
...
pb <-
  txtProgressBar(min = 1,
                 max = nrow(hvar42Y_isi),
                 style = 3)#nrow(hvar42Y_isi)
progress <- function(n)
  setTxtProgressBar(pb, n)
opts <- list(progress = progress)
...
FW_results <-
  foreach(
    i = 1:nrow(hvar42Y_isi),
    .options.snow = opts,
    .combine = 'rbind'
  ) %dopar%  FireWeatherLongTerm(i)#, .combine = 'rbind'
stopCluster(cl)
end_time <- Sys.time()
end_time - start_time
...
FW_results = read.csv(
  'D:\\000_collections\\010_Nighttime Burning\\011_Data\\015_LongTermFireWeather\\results_hr\\FW_results.csv'
)
...
## daytime high
FW.d.42 = FW_results.f21 %>% dplyr::select('vpd_daymax',
                                           'isi_daymax',
                                           'ffmc_daymax',
                                           'temp_daymax',
                                           'rh_daymin',
                                           'id',
                                           'ts')
FW.d.a = FW_results.l21 %>% dplyr::select('vpd_daymax',
                                          'isi_daymax',
                                          'ffmc_daymax',
                                          'temp_daymax',
                                          'rh_daymin',
...
ggplot(data = xx2, aes(x = ts, y = value, fill = variable)) + geom_boxplot(size =
                                                                             0.5) +
...
## nighttime low
FW.n.42 = FW_results.f21 %>% dplyr::select(
  'vpd_nightmin',
  'isi_nightmin',
  'ffmc_nightmin',
  'temp_nightmin',
...
FW.n.a = FW_results.l21 %>% dplyr::select(
  'vpd_nightmin',
  'isi_nightmin',
  'ffmc_nightmin',
  'temp_nightmin',
...
ggplot(data = xx3, aes(x = ts, y = value, fill = variable)) + geom_boxplot(size =
                                                                             0.5) +
  geom_line(aes(group = id), position = position_dodge(0.2), alpha = 0.08) +
  geom_point(
    aes(color = variable, group = id),
...
res <-
  wilcox.test((FW.d.42 %>% dplyr::select(VPD_Dmax))$VPD_Dmax,
              (FW.d.a %>% dplyr::select(VPD_Dmax))$VPD_Dmax,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((FW.n.42 %>% dplyr::select(VPD_Nmin))$VPD_Nmin,
              (FW.n.a %>% dplyr::select(VPD_Nmin))$VPD_Nmin,
              paired = TRUE,
              alternative = 'greater'
  )
res
...
res <-
  wilcox.test((FW.d.42 %>% dplyr::select(ISI_Dmax))$ISI_Dmax,
              (FW.d.a %>% dplyr::select(ISI_Dmax))$ISI_Dmax,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((FW.n.42 %>% dplyr::select(ISI_Nmin))$ISI_Nmin,
              (FW.n.a %>% dplyr::select(ISI_Nmin))$ISI_Nmin,
              paired = TRUE,
              alternative = 'greater'
  )
res
...
res <-
  wilcox.test((FW.d.42 %>% dplyr::select(FFMC_Dmax))$FFMC_Dmax,
              (FW.d.a %>% dplyr::select(FFMC_Dmax))$FFMC_Dmax,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((FW.n.42 %>% dplyr::select(FFMC_Nmin))$FFMC_Nmin,
              (FW.n.a %>% dplyr::select(FFMC_Nmin))$FFMC_Nmin,
              paired = TRUE,
              alternative = 'greater'
  )
res
...
res <-
  wilcox.test((FW.d.42 %>% dplyr::select(T_Dmax))$T_Dmax,
              (FW.d.a %>% dplyr::select(T_Dmax))$T_Dmax,
              paired = TRUE,
              alternative = 'greater'
  )
res
res <-
  wilcox.test((FW.n.42 %>% dplyr::select(T_Nmin))$T_Nmin,
              (FW.n.a %>% dplyr::select(T_Nmin))$T_Nmin,
              paired = TRUE,
              alternative = 'greater'
  )
res
...
res <-
  wilcox.test((FW.d.42 %>% dplyr::select(RH_Dmin))$RH_Dmin,
              (FW.d.a %>% dplyr::select(RH_Dmin))$RH_Dmin,
              paired = TRUE,
              alternative = 'less'
  )
res
res <-
  wilcox.test((FW.n.42 %>% dplyr::select(RH_Nmax))$RH_Nmax,
              (FW.n.a %>% dplyr::select(RH_Nmax))$RH_Nmax,
              paired = TRUE,
              alternative = 'greater'
  )
res
...
plotx <-
  rbind(xx, xx2, xx3) %>% filter(variable %in% c('FWI', 'BUI', 'DMC', 'FFMC_Nmin', "VPD_Nmin"))

plotx$variable <-
  factor(plotx$variable,
         levels = c('FWI', 'BUI', 'DMC', 'FFMC_Nmin', "VPD_Nmin"))
...
plotx <-
  rbind(xx, xx2, xx3) %>% filter(
    variable != 'FWI' &
      variable != 'BUI' &
      variable != 'DMC' & variable != 'FFMC_Nmin' & variable != 'VPD_Nmin'
  )
plotx$variable <-
  factor(
    plotx$variable,
