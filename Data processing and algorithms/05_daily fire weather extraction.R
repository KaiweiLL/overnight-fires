# This script is used to extract and calculate daily fire weather
# similar to 04_hrly fire cycle_hrly fire weather.R
...
GOES17 <-
  GOES17 %>% filter(
    long >= raster.brick@extent@xmin &
      long <= raster.brick@extent@xmax &
...
BUI <-
  mixedsort(dir('NA_BUI_daily\\', full.names = T, pattern = '(.nc)$'))
DC <-
  mixedsort(dir('NA_DC_daily\\', full.names = T, pattern = '(.nc)$'))
DMC <-
  mixedsort(dir('NA_DMC_daily\\', full.names = T, pattern = '(.nc)$'))
FWI <-
  mixedsort(dir('NA_FWI_daily\\', full.names = T, pattern = '(.nc)$'))

# locate the corresponding fire weather file
...
f <- function(i) {
  ...
  if (dim(GOES_fireinfo)[1] > 0) {
    ...
    ext.val.bui <-
      raster::extract(var.bui.nc, shp.indi, weight = T)[[1]]
    nr <- dim(ext.val.bui)[2] - 1
    ext.val.bui <- data.frame(ext.val.bui)[, 1:nr]
    ext.val.dmc <-
      data.frame(raster::extract(var.dmc.nc, shp.indi, weight = T)[[1]])[, 1:nr]
    ext.val.dc <-
      data.frame(raster::extract(var.dc.nc, shp.indi, weight = T)[[1]])[, 1:nr]
    ext.val.fwi <-
      data.frame(raster::extract(var.fwi.nc, shp.indi, weight = T)[[1]])[, 1:nr]
    ...
    out.val.bui <-
      cbind(rowMeans(out.val.bui, na.rm = T),
            apply(out.val.bui, 1))
    out.val.dmc <-
      cbind(rowMeans(out.val.dmc, na.rm = T),
            apply(out.val.dmc, 1))
    out.val.dc <-
      cbind(rowMeans(out.val.dc, na.rm = T),
            apply(out.val.dc, 1))
    out.val.fwi <-
      cbind(rowMeans(out.val.fwi, na.rm = T),
            apply(out.val.fwi, 1))
    
    colnames(out.val.bui) <- c('bui.mean')
    colnames(out.val.dmc) <- c('dmc.mean')
    colnames(out.val.dc) <- c('dc.mean')
    colnames(out.val.fwi) <- c('fwi.mean')
    ...
    GOES_fireinfo <- GOES_fireinfo %>% group_by(year, day) %>%
      summarise(
        lat = mean(lat),
        long = mean(long),
        frp.mean = mean(frp.mean),
        frp.total = mean(frp.total),
        spotsnum = mean(spotsnum)
      )
    ...
    tz <-
      tz_offset(
        ...
        tz_lookup_coords(shp.indi$lat, shp.indi$long, method = "accurate")
      )
    ...
    GOES_fireinfo.var <-
      GOES_fireinfo.var %>% dplyr::select(year:spotsnum, biome:inci_name, everything())
